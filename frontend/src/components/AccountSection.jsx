...
import { User, ShieldCheck, Heart, ShieldAlert, Sparkles, Flame, Scale, Check, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function AccountSection({
  ...
  return (
    <section className="bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl transition-all">
      {/* Section Header */}
      <div className="flex items-center justify-between cursor-pointer select-none" onClick={() => setIsExpanded(!isExpanded)}>
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center justify-center shadow-sm">
        ...
        <button
          type="button"
          className="p-2 rounded-xl bg-slate-800/80 hover:bg-slate-700 text-slate-400 hover:text-white transition-colors active:scale-[0.96]"
        >
          {isExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
        </button>
      </div>

      {/* Expanded Profile Configuration Body */}
      {isExpanded && (
        <div className="mt-6 pt-6 border-t border-slate-800/80 space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 space-y-4">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl px-3 py-2 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/40 tabular-nums"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl px-3 py-2 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950/80 border border-slate-700/80 rounded-xl px-3 py-2 text-xs font-semibold text-white focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
                  >
              ...
              <div>
                <label className="block text-xs font-bold text-slate-300 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                  <Heart className="w-3.5 h-3.5 text-rose-400" /> Medical Conditions (Clinical Guardrails)
                </label>
                  ...
                  {conditionsList.map((cond) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all active:scale-[0.96] ${
                          isSelected
                  ...
                  {allergiesList.map((all) => {
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1 rounded-xl text-xs font-bold flex items-center gap-1.5 transition-all active:scale-[0.96] ${
                          isSelected
            ...
            <div className="lg:col-span-5 bg-slate-950/60 border border-slate-800/80 rounded-2xl p-5 flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-1.5">
                    <Flame className="w-4 h-4 text-emerald-400" /> Active Nutritional Matrix
                  </span>
                  {loadingMatrix && (
                    <span className="text-[11px] text-emerald-400 font-semibold animate-pulse">Syncing...</span>
                  )}
                </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800">
                      <div className="flex items-center justify-between">
                        <div>
                          <span className="text-[11px] text-slate-400 font-semibold block">Target Calories</span>
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-[10px] text-slate-500 mt-1">BMR: {Math.round(m.bmr_kcal)} | TDEE: {Math.round(m.tdee_kcal)}</div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3">
                        ...
                        <div className="flex justify-between text-[11px] font-semibold mt-1.5 text-slate-300 tabular-nums">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-[11px]">
                      <div className="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800">
                        ...
                        <span className="font-bold text-sky-400 tabular-nums">&lt; {g?.sodium_ceiling_mg} mg</span>
                      </div>
                      <div className="p-2.5 rounded-lg bg-slate-900/60 border border-slate-800">
                        <span className="text-slate-500 block">Sat. Fat Cap</span>
                        <span className="font-bold text-rose-400 tabular-nums">&lt; {Math.round(g?.saturated_fat_max_pct * 100)}% kcal</span>
                      </div>
                    </div>

                    {userMatrix.exclusion_mask?.length > 0 && (
                      <div className="p-2.5 rounded-xl bg-rose-950/20 border border-rose-500/30 text-[11px]">
                        <span className="font-bold text-rose-400 block mb-1">Strict Exclusion Mask:</span>
                        <span className="font-mono text-rose-200">{userMatrix.exclusion_mask.join(', ')}</span>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="text-xs text-slate-500 p-6 text-center">Loading matrix...</div>
                )}
              </div>
              ...
              <div className="mt-4 pt-3 border-t border-slate-800/80 flex justify-end">
                <button
                  ...
                  className="px-5 py-2 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-slate-950 font-extrabold text-xs shadow-md transition-all active:scale-[0.96] flex items-center gap-1.5"
                >
